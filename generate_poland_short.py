...
# Голос: русский мужской, дружелюбный тон
...
ANGLES = [
    ...
    "сколько реально стоит жизнь в Польше",
    "что изменилось в законах за последнее время",
...
FALLBACK_METADATA = VideoMetadata(
    title="5 вещей, которые нужно сделать сразу после переезда в Польшу 🇵🇱 #shorts",
    description=(
        "Переехал в Польшу? Вот 5 шагов, которые нужно сделать в первый месяц. "
        "Сохрани, чтобы не забыть!\n\n"
        "#польша #переезд #легализация #явпольше #shorts #лайфхак"
    ),
...
def _validate_script(parts: List[ScriptPart]) -> bool:
    ...
    # Минимум 40% фраз с конкретикой (цифры, названия, действия)
    concrete_markers = re.compile(
        ...
        r'зайди|оформи|подай|получи|открой|зарегистрируй|найди|'
        r'бесплатн|скидк|стоит|цена|зарплат|'
...
def _fallback_script() -> tuple:
    parts = [
        ...
        ScriptPart("Первое — получи PESEL. Без него не откроешь счёт в банке и не оформишь карту побыту."),
        ScriptPart("Идёшь в ближайший уженд гмины с паспортом и договором аренды. Занимает двадцать минут."),
        ScriptPart("Второе — зарегистрируйся в NFZ. Это даёт бесплатную медицину: врач, больница, скорая."),
        ScriptPart("Работодатель обязан тебя застраховать. Проверь, что он платит взносы в ZUS."),
        ScriptPart("Третье — открой банковский счёт. Без него не получишь зарплату и не оплатишь квартиру."),
        ScriptPart("Рекомендую mBank или PKO — у них есть приложения на русском и английском."),
...
def call_groq_for_script() -> tuple:
    ...
    system_prompt = (
        "Ты — русскоязычный блогер, живущий в Польше. Ты ведёшь канал 'Я в Польше' "
        "и помогаешь русскоязычным людям с переездом, легализацией и жизнью в Польше. "
        "Ты пишешь сценарии с РЕАЛЬНЫМИ полезными советами: конкретные адреса, суммы в злотых, "
        "названия документов, пошаговые инструкции, лайфхаки по экономии. "
        "НИКОГДА не пиши пустых фраз-наполнителей вроде 'Это невероятно' или 'Ты не поверишь'. "
        "Каждая фраза = конкретный совет, факт, сумма или действие. "
        "Польские названия пиши кириллицей (как произносятся), чтобы озвучка звучала правильно. "
        "Например: PESEL → ПЕСЕЛЬ, Urząd Gminy → уженд гмины, NFZ → НФЗ, ZUS → ЗУС. "
...
1. Первая фраза — мощный хук: шокирующий факт с цифрой или провокационный вопрос про жизнь в Польше.
2. КАЖДАЯ фраза должна содержать КОНКРЕТНУЮ пользу: сумму в злотых, название документа, адрес, срок, пошаговое действие.
3. НИКАКИХ пустых фраз. Запрещено: "Это невероятно", "Ты не поверишь", "Мой день начинается рано".
4. Каждая фраза = 1–2 предложения, 10–25 слов.
5. Используй «ты»-обращение, как друг советует другу, который только переехал.
6. Финал — призыв сохранить видео и написать свой опыт в комментариях.
7. 10–14 частей всего (для 45–60 секунд видео).
8. Язык — живой разговорный русский. Польские термины пиши КИРИЛЛИЦЕЙ (как звучат): PESEL → ПЕСЕЛЬ, złotych → злотых, Urząd → уженд, ZUS → ЗУС.

ПРИМЕР ХОРОШЕЙ ФРАЗЫ: "Оформи ПЕСЕЛЬ в уженде гмины — без него не откроешь счёт в банке и не получишь пособие на ребёнка."
ПРИМЕР ПЛОХОЙ: "Это невероятно!" или "Ты должен это знать."

Формат — строго JSON:
{{
  "title": "Цепляющий заголовок (до 70 символов) с эмодзи 🇵🇱 и #shorts",
  "description": "Описание для YouTube (2–3 строки) с хештегами",
  "tags": ["польша", "явпольше", "shorts", ...ещё 4-7 тематических тегов],
