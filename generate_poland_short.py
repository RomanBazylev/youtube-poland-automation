...
# Темы контента — практика + культура + места + кухня
TOPICS = [
    # Практические
...
_FALLBACK_POOL = [
    ...
    # [5] — Краков — места, которых нет в путеводителях
    [
        ...
        ScriptPart("Дальше поднимись на Копец Костюшко — насыпной холм с панорамой на весь город и Татры вдалеке."),
        ...
        ScriptPart("Вечером загляни в Забломе — бывший промышленный район, который превратился в арт-квартал."),
        ScriptPart("Здесь галереи, крафтовые бары и уличная еда — жизнь кипит до позднего вечера."),
        ScriptPart("Если был в Кракове — напиши, что понравилось больше всего. Сохрани и подпишись!"),
    ],
]
...
_FALLBACK_META_POOL = [
    ...
    VideoMetadata(
        title="Аренда квартиры в Польше — реальные цены 🏠 #shorts",
    ...
    VideoMetadata(
        title="Польская кухня — 5 блюд, которые нужно попробовать 🥟 #shorts",
        description="Перо́ги, жу́рек, обважа́нек — польская кухня удивляет!\n\n#польша #кухня #еда #перо́ги #явпольше #shorts",
        tags=["польша", "кухня", "еда", "пероги", "журек", "явпольше", "shorts", "путешествия"],
    ),
    VideoMetadata(
        title="Краков — места, которых нет в путеводителях 🏰 #shorts",
        description="Казимеж, Копец Костюшко, подземный музей — скрытые жемчужины Кракова.\n\n#краков #польша #путешествия #явпольше #shorts",
...
def call_groq_for_script() -> tuple:
    ...
    system_prompt = (
        ...
        "Канал о ВСЁМ, что связано с Польшей: практические советы, культура, традиции, "
        "интересные места, кухня, история, события. Ты рассказываешь увлекательно и живо.\n\n"
        ...
        "Фразы идут как шаги одной истории: зацепка → развитие → кульминация → вывод.\n\n"
        ...
        "АКТУАЛЬНОСТЬ: используй самую свежую информацию. Сейчас 2026 год. "
        "НЕ упоминай устаревшие суммы, отменённые программы или старые законы. "
        "Если не уверен в конкретной цифре — используй диапазон ('от X до Y').\n\n"
        "КОНКРЕТИКА: конкретные названия, цифры, адреса, даты, имена. "
        "Для практических тем — суммы в злотых, документы, сроки. "
        "Для культурных тем — названия мест, блюд, традиций, исторические факты. "
        ...
        "pierogi → пероги, żurek → журек, bigos → бигос, obwarzanek → обважанек.\n\n"
...
- Город: {city}
- Стиль: {angle}
- Год: 2026 (используй самую актуальную информацию)
...
- Используй самую актуальную информацию на момент 2026 года.
  ...
  "parts": [
    ...
    body["messages"].append({
        ...
        "content": (
            ...
            "4. Информация актуальна на 2026 год.\n"
